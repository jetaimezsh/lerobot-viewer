...
7. 可选：勾选"限制帧数"以快速验证
8. 可选：在"回测环境变量"中设置 `CUDA_VISIBLE_DEVICES=0`、`CUDA_VISIBLE_DEVICES=0,1` 或其他环境变量
9. 点击"运行回测"；多个任务会进入后台队列并依次执行，可在队列表格中刷新查看状态

环境变量只在回测任务执行期间临时应用，并会写入任务/历史摘要；敏感变量值会脱敏。`CUDA_VISIBLE_DEVICES` 应在模型加载前设置，如果 torch/CUDA 已在后端进程中初始化，建议重启后端后再运行。
